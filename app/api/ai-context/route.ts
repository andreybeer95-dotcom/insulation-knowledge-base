...
export async function GET(request: NextRequest) {
  ...
  const fastBrandCylinder = (() => {
    const brandSpecs = [
      { brand: 'BOS', test: /\bbos(?:[-\s]?pipe)?\b|\bбос\b/i, ruleNeedle: 'bos' },
      { brand: 'ISOTEC', test: /\bisotec\b|\bизотек\b/i, ruleNeedle: 'isotec' },
      { brand: 'BAZTECH', test: /\bbaztech\b|\bбазтех\b/i, ruleNeedle: 'baztech' },
    ]
    return brandSpecs.find((spec) => spec.test.test(rawQuery))
  })()

  const fastHasCylinderIntent = /цилиндр|скорлуп|отвод|трубопровод|изоляц/i.test(rawQuery)
  if (compactMode && toolResponseMode && fastBrandCylinder && fastHasCylinderIntent && requestedSizeNumbers.length >= 2) {
    const [firstSize, secondSize] = requestedSizeNumbers
    const matchesFastSize = (item: { name: string | null; article: string | null }) => {
      const text = `${item.name || ''} ${item.article || ''}`
      const direct = new RegExp(`(^|\\D)${firstSize}\\s*[xх*\\-]\\s*${secondSize}(\\D|$)`, 'i')
      const reversed = new RegExp(`(^|\\D)${secondSize}\\s*[xх*\\-]\\s*${firstSize}(\\D|$)`, 'i')
      const dotted = new RegExp(`(^|\\D)${firstSize}\\.${secondSize}(\\D|$)`, 'i')
      return direct.test(text) || reversed.test(text) || dotted.test(text)
    }
    const isFastAccessory = (name?: string | null) => /отвод|колено|заглуш|пробк|тройник|переход|сегмент/i.test(name || '')

    const { data: fastNomData } = await supabase
      .from('nomenclature_1c')
      .select('id, code, article, name, brand')
      .eq('brand', fastBrandCylinder.brand)
      .ilike('name', `%${firstSize}%`)
      .ilike('name', `%${secondSize}%`)
      .limit(80)

    const fastItems = ((fastNomData ?? []) as any[]).filter(matchesFastSize)
    const fastMainItems = fastItems.filter((item) => !isFastAccessory(item.name)).slice(0, 8)
    const fastAccessories = fastItems.filter((item) => isFastAccessory(item.name)).slice(0, 6)

    if (fastMainItems.length > 0) {
      const { data: fastRulesData } = await supabase
        .from('selection_rules')
        .select('rule_name, rule_text, is_prohibition, category, priority, condition')
        .or(`condition.ilike.%${fastBrandCylinder.ruleNeedle}%,rule_name.ilike.%${fastBrandCylinder.brand}%`)
        .order('priority', { ascending: true })
        .limit(6)

      const fastQuestions = [
        'Уточните точный вариант: плотность/серия и покрытие.',
        'Для улицы проверьте защитное покрытие или окожушку.',
      ]
      ...
      const fastContext = [
        ...
        '',
        '## Основные позиции 1С',
        ...fastMainItems.map((item) => {
          const article = item.article ? ` | article: ${item.article}` : ''
          return `- **${item.name}** (код 1С: ${item.code || '—'}${article})`
        }),
        '',
        '## Сопутствующие',
        ...(fastAccessories.length > 0
          ? fastAccessories.map((item) => `- **${item.name}** (код 1С: ${item.code || '—'})`)
          : ['- Проверить отводы/окожушку/крепёж по проекту.']),
        '',
        '## Что уточнить',
        ...fastQuestions.map((question) => `- ${question}`),
        '',
        '## Правила ответа',
      ...
      return NextResponse.json({
        context: fastContext,
        main_items: fastMainItems.map((item) => ({
          code: item.code,
        ...
        accessories: fastAccessories.map((item) => ({
          code: item.code,
          article: item.article,
          name: item.name,
          brand: item.brand,
        ...
        rules: (fastRulesData ?? []).map((rule: any) => ({
          name: rule.rule_name,
          text: rule.rule_text,
        ...
        meta: {
          compact: true,
          fast_path: 'brand_cylinder',
          products_count: 0,
          nomenclature_count: fastMainItems.length,
          accessories_count: fastAccessories.length,
          strict_invoice: false,
          invoice_lines_count: 0,
          missing_items_count: 0,
          rules_count: fastRulesData?.length ?? 0,
          chunks_count: 0,
        },
      })
    }
