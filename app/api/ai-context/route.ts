...
export async function GET(request: NextRequest) {
  ...
  if (hasAnySystemQueryForContext) {
    selection_guidance.answer_policy = [
      ...
      'Не добавлять BASWOOL, ROCKWOOL, фасадный крепеж, пленки или общую сопутку вне состава системы, если менеджер прямо не попросил аналог/замену.',
      'Материалы с кодом 1С из системной карточки можно ставить в счет; роли с пометкой "код не найден", "по проекту" или "не ставить без проверки" — вынести в уточнения/проектные слои.',
      'Для площади считать только те роли, где в системном правиле указан расход/коэффициент; уклонку, крепеж, стяжку, балласт и основание считать только по проекту.',
      ...selection_guidance.answer_policy,
    ]
    selection_guidance.analog_policy = [
      'Аналоги и замены для конкретной системы показывать только если менеджер прямо просит замену; иначе не уводить ответ от системы.',
      'Если предлагается замена материала системы, написать, что это отступление от системного решения и его нужно согласовать по проекту.',
      ...selection_guidance.analog_policy,
    ]
  ...
  const compactFormattedContext = [
    ...
    ...(hasAnySystemQueryForContext
      ? [
          '- Для конкретной NAV.TN системы отвечать только по составу системы из rules.',
          '- Не добавлять BASWOOL/ROCKWOOL и общие аналоги, если менеджер прямо не попросил замену.',
          '- Проектные слои, основание, уклонку, крепеж, стяжку и балласт не считать без проектных данных.',
        ]
  ...
  const strictCodeRule = {
    ...
    rule_text: [
      ...
      'Код 1С можно писать только если он явно пришел из JSON-полей инструмента: main_items, requested_invoice_items, invoice_lines.item, accessories, analogs или rules.',
      'Коды внутри rules разрешены только потому, что системные правила формируются после сверки с nomenclature_1c; если правило прямо пишет, что код не найден или роль без кода, такой материал в счет не ставить.',
